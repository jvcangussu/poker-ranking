-- Permite validar totais também em in_adjustment (após reenvio individual),
-- com a mesma regra de negócio que em in_review.
...
BEGIN
  ...
  IF v_status NOT IN ('in_review', 'in_adjustment') THEN
    RAISE EXCEPTION 'Validação só está disponível com a partida em análise ou em ajuste.';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Todos os participantes precisam ter enviado para análise antes de validar.';
  END IF;
